...
const CHARMAP = {
    ...
    accented2: { y: 52, height: 10, xSpacing: 1, chars: [
...
function getWordMetrics(text) {
    text = text.replace(new RegExp('\n', 'g'), ' \n '); // Make line breaks their own word
    ...
    for(var w = 0; w < words.length; w++) {
        ...
        if(charIndex >= 0) { // Single char or icon
...
function blotText(params) {
    ...
    var meta = {
        x: left, y: 0, 
        width: blotMetrics.blottedWidth, height: blotMetrics.blottedHeight,
        totalChars: 0, lines: [] 
    };
    for(var l = 0; l < blotMetrics.charLines.length; l++) {
        var caretX = left;
        var lineCharCount = blotMetrics.charLines[l].length;
        meta.totalChars += lineCharCount;
        var lineMetrics = { x: caretX, y: l * lineHeight, height: lineHeight, charsX: [] };
        var char;
        for(var c = 0; c < lineCharCount; c++) {
            var caretY = lineMetrics.y;
            char = blotMetrics.charLines[l][c];
            lineMetrics.charsX.push(caretX);
            if(char !== space) canvas.drawImage(
                fontSheet, xArr[char], yArr[char], wArr[char], hArr[char], caretX, caretY + oyArr[char]
            );
            caretX += wArr[char];
        }
        lineMetrics.width = caretX;
        meta.lines.push(lineMetrics);
    }
    return { canvas: canvas.canvas, meta };
}

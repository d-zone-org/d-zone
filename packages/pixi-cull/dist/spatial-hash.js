	...
	_createClass(SpatialHash, [
		...
		},
		{
			...
			value: function addContainer(container, staticObject) {
				...
				try {
					for (
						var _iterator = container.children[Symbol.iterator](), _step;
		...
		},
		{
			...
			value: function updateObjects() {
				...
				if (this.dirtyTest) {
					...
					try {
						for (
							var _iterator3 = this.objects[Symbol.iterator](), _step3;
					...
					try {
						for (
							var _iterator4 = this.containers[Symbol.iterator](), _step4;
							...
							try {
								for (
									var _iterator5 = container.children[Symbol.iterator](),
										_step5;
					...
					try {
						for (
							var _iterator6 = this.containers[Symbol.iterator](), _step6;
					...
					for (var x = xStart; x <= xEnd; x++) {
						...
						if (entry) {
							if (simpleTest) {
								...
								try {
									for (
										var _iterator7 = entry[Symbol.iterator](), _step7;
						...
						if (entry) {
							for (var i = 0; i < entry.length; i++) {
								...
								if (simpleTest) {
									...
									if (
										_AABB.x + _AABB.width > _AABB.x &&
										_AABB.x < _AABB.x + _AABB.width &&
										_AABB.y + _AABB.height > _AABB.y &&
										_AABB.y < _AABB.y + _AABB.height
									) {
										if (callback(object)) {
											return true
										}
		...
		},
		{
			...
			value: function stats() {
				...
				try {
					for (
						var _iterator8 = this.containers[Symbol.iterator](), _step8;
